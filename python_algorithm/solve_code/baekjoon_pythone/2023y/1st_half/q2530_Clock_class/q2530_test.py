...
def sec_to_clock_obj(param_seconds: int) -> Clock:
    ...
    # Calculate days
    days, reminded_days = divmod(param_seconds, one_day_seconds)
    hour, reminded_hours = divmod(reminded_days, one_hour_seconds)
    minute, second = divmod(reminded_hours, one_minute_seconds)

    print(f"days:{days}, hour: {hour}, minute: {minute}, second: {second}")
